...
GuiModManager::~GuiModManager() {
  // Wait for any running async operations to complete before destroying
  if (_asyncResponse_.valid()) {
    LogInfo << "Waiting for async operation to complete..." << std::endl;
    // Wait with timeout to prevent hanging during cleanup
    auto status = _asyncResponse_.wait_for(std::chrono::seconds(5));
    if (status == std::future_status::timeout) {
      LogWarning << "Async operation timed out after 5 seconds - may cause issues" << std::endl;
    }
  }
}

void GuiModManager::waitForAsyncOperation() {
  // If there's already an async operation running, wait for it with timeout
  if (_asyncResponse_.valid()) {
    auto status = _asyncResponse_.wait_for(std::chrono::milliseconds(100));
    if (status == std::future_status::timeout) {
      LogWarning << "Previous async operation still running, waiting up to 3 seconds..." << std::endl;
      status = _asyncResponse_.wait_for(std::chrono::seconds(3));
      if (status == std::future_status::timeout) {
        LogError << "Previous async operation timed out - forcing new operation anyway" << std::endl;
      }
    }
  }
}
...
void GuiModManager::applyMod(const std::string &modName_) {
  ...
  // Use only the Title ID subfolder if it exists
  std::string titleId = _gameBrowser_.getModManager().getTitleId();
  if (!titleId.empty()) {
    std::string titleIdPath = GenericToolbox::joinPath(modPath, titleId);
    if (GenericToolbox::doesPathIsFolder(titleIdPath)) {
      modPath = titleIdPath;
      LogInfo << "Using Title ID folder: " << titleId << std::endl;
    }
  }
  ...
  for(size_t iFile = 0 ; iFile < modFilesList.size() ; iFile++){
    ...
    std::string installPath = GenericToolbox::joinPath(
        _gameBrowser_.getModManager().fetchCurrentPreset().installBaseFolder,
        GenericToolbox::joinPath(titleId, modFilesList[iFile])
    );
...
void GuiModManager::getModStatus(const std::string &modName_, bool useCache_) {
  ...
  // Use only the Title ID subfolder if it exists
  std::string titleId = modManager.getTitleId();
  if (!titleId.empty()) {
    std::string titleIdPath = GenericToolbox::joinPath(modPath, titleId);
    if (GenericToolbox::doesPathIsFolder(titleIdPath)) {
      modPath = titleIdPath;
      LogInfo << "Checking Title ID folder: " << titleId << std::endl;
    }
  }

  int sameFileCount = 0;

  modCheckMonitor.currentFile = "menus/mods/listing_mod_files"_i18n;
  ...
  for( size_t iFile = 0 ; iFile < modFileList.size() ; iFile++ ){
    ...
    std::string dstFilePath = GenericToolbox::joinPath(
        modManager.fetchCurrentPreset().installBaseFolder,
        GenericToolbox::joinPath(titleId, modFileList[iFile])
    );
  ...
  if     ( modFileList.empty()           ) cacheEntry.statusStr = "menus/mods/status_no_file"_i18n;
  else if( cacheEntry.applyFraction == 0 ) cacheEntry.statusStr = "menus/mods/status_inactive"_i18n;
  else if( cacheEntry.applyFraction == 1 ) cacheEntry.statusStr = "menus/mods/status_active"_i18n;
  else cacheEntry.statusStr = "menus/mods/status_partial"_i18n + " (" + GenericToolbox::joinAsString("/", sameFileCount, modFileList.size()) + ")";
...
void GuiModManager::removeMod(const std::string &modName_){
  ...
  if (!titleId.empty()) {
    ...
    if (GenericToolbox::doesPathIsFolder(titleIdPath)) {
      modPath = titleIdPath;
      LogInfo << "Removing from Title ID folder: " << titleId << std::endl;
    }
  }
  ...
  for(auto &modFile : modFileList){
    ...
    std::string dstFilePath = GenericToolbox::joinPath(
        _gameBrowser_.getModManager().fetchCurrentPreset().installBaseFolder,
        GenericToolbox::joinPath(titleId, modFile)
    );
...
void GuiModManager::startApplyModThread(const std::string& modName_) {
  ...
  // Wait for any previous async operation before starting new one
...
bool GuiModManager::applyModFunction(const std::string& modName_){
  ...
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/applying_mod"_i18n);
  ...
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/checking_applied_mod"_i18n);
...
bool GuiModManager::applyModPresetFunction(const std::string& presetName_){
  ...
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/removing_all_mods"_i18n);
  _loadingPopup_.getMonitorView()->resetMonitorAddresses();
  ...
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/applying_mod_preset"_i18n);
  _loadingPopup_.getMonitorView()->setProgressColor(GenericToolbox::Borealis::greenNvgColor);
  _loadingPopup_.getMonitorView()->resetMonitorAddresses();
  ...
  _loadingPopup_.getMonitorView()->setProgressColor(GenericToolbox::Borealis::blueNvgColor);
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/checking_all_mods"_i18n);
...
bool GuiModManager::removeModFunction(const std::string& modName_){
  ...
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/removing_mod"_i18n);
  _loadingPopup_.getMonitorView()->setProgressColor(GenericToolbox::Borealis::redNvgColor);
  _loadingPopup_.getMonitorView()->resetMonitorAddresses();
  _loadingPopup_.getMonitorView()->setTitlePtr(&modName_);
  _loadingPopup_.getMonitorView()->setSubTitlePtr( &modRemoveMonitor.currentFile );
  ...
  LogWarning << "Checking: " << modName_ << std::endl;
  _loadingPopup_.getMonitorView()->setProgressColor( GenericToolbox::Borealis::blueNvgColor );
  _loadingPopup_.getMonitorView()->setHeaderTitle("menus/mods/checking_mod"_i18n);
  _loadingPopup_.getMonitorView()->resetMonitorAddresses();
  _loadingPopup_.getMonitorView()->setTitlePtr(&modName_);
  _loadingPopup_.getMonitorView()->setSubTitlePtr( &modCheckMonitor.currentFile );
  _loadingPopup_.getMonitorView()->setProgressFractionPtr( &modCheckMonitor.progress );
